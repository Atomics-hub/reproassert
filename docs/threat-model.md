...
| Malicious v0.2 replay bundle | Canonical duplicate-key-free <=2 MiB input; self-hash; exact case/source/candidate/declared-tool/fingerprint binding; embedded dependency plan re-parsed; only the packaged trusted runner tag is accepted; rebuilt dependency tree and immutable image ID must match | Bundle is unsigned; the publisher-declared tool revision is unauthenticated; image IDs are architecture-local; trusted wheel download still uses bridge egress rather than a destination ACL |
...
| False product claim | Explicit claim levels and report limitations; `repeatable_base_failure` has a narrow definition; L2 requires executed controls plus reviewer consensus | Authentic scored runs, benchmark rates, and maintainer acceptance remain separate missing evidence gates at 0/20 and zero |
